...
def xml_to_csv(path, train_eval_ratio=0.7):
    ...
    for i, xml_file in enumerate(xml_files):
        ...
        for member in root.findall('object'):
            print(member[0].text)
            value = (f"images/{dataset}/{root.find('filename').text}",
        ...
        print(f'moving {img_path} to {value[0]}')
        shutil.move(img_path, 'workspace/castelloli/' + value[0])
...
def create_tf_example(group, path):
    with tf.gfile.GFile(os.path.join(path, 'workspace/castelloli/{}'.format(group.filename)), 'rb') as fid:
...
def main(_):
    ...
    train_examples, eval_examples = xml_to_csv(args.xml_dir)
    examples = {'train': train_examples, 'eval': eval_examples}
    
    for dataset in ['train', 'eval']:
        output_tf = f'{args.output_path}/{dataset}.record'
        ...
        if args.csv_path is not None:
            output_csv = f'{args.csv_path}/{dataset}.csv'
