...
def main():
    ...
    for frame_id in frames_with_objects:
        ...
        print(f'[{frame_id}] Took {infer_latency:.2f} seconds to process {len(composed_frames)} frames ({1/infer_latency:.2f} fps) -> {cf2method}.')
        ...
        for method_id, method in enumerate(cf2method):
            ...
            for i in range(min(len(boxes), max_boxes)):
                ...
                draw_detection(
                    composed_frame,
                    [
                        infer_left, infer_top,
                        infer_right, infer_bottom
                    ],
                    label,
                    score)
                ...
                if method == 'full_frame':
                    (left, right, top, bottom) = (
                                                int(xmin * frame_bgr.shape[1]), int(xmax * frame_bgr.shape[1]),
                                                int(ymin * frame_bgr.shape[0]), int(ymax * frame_bgr.shape[0]))
                ...
                if any([c < 0 for c in obj_coords]) or \
                   obj_coords[2] > frame_bgr.shape[1] or \
                   obj_coords[3] > frame_bgr.shape[0]:
                    import pdb; pdb.set_trace()
