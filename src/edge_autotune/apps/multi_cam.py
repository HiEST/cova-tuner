...
def motion_rois(frames, rets, motionDetector, min_area):
    collage = None
    collage_rois = []

    valid_rois = []
    for video_id, _ in enumerate(frames):
    ...
    return collage, collage_rois


def detect_frame(frame, detector, label_map, input_size, min_score=0.5, max_boxes=20):
    preds = run_detector(detector, frame, input_size=input_size) 
...
def detect_collage(collage, collage_rois, frames, detector, label_map, input_size, min_score=0.5, max_boxes=20):
...
def main():
    ...
    # Motion Detection
    args.add_argument("--disable-motion-detection", action="store_true", help="Disables motion detection and full frames are passed to the NN")
    ...
    if not config.disable_motion_detection:
        background = [
            Background(
    ...
    while True: 
        ...
        if not config.disable_motion_detection:
            collage, collage_rois = motion_rois(frames, rets, motionDetector, config.min_area)
        else:
            collage, collage_rois = [None, []]
        ...
        if config.disable_motion_detection:
            for video_id, _ in enumerate(frames):
                if not rets[video_id]:
                    continue
                frames[video_id] = detect_frame(frames[video_id], detector, label_map, input_size, min_score)
            ...
            if config.no_infer or collage is None:
                ...
                collage, frames = detect_collage(collage, collage_rois, frames, detector, label_map, input_size, min_score)
