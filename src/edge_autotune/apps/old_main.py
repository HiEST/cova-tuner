...
def main():
    ...
    args.add_argument("--resize-height", type=int, default=300, help="New height to resize images keeping aspect ratio")
    args.add_argument("--jump-to", type=int, default=0, help="Frame to jump to")
    args.add_argument("--square-input", action="store_true", default=False, help="Make input images 1:1 ratio")
    args.add_argument("--pad-input", action="store_true", default=False, help="Pad input to make images 1:1 ratio")
    ...
    args.add_argument("--max-boxes", type=int, default=10, help="maximumim number of bounding boxes per frame")
    ...
    while True:
        ...
        if config.resize_height is not None:
            frame = imutils.resize(frame, height=config.resize_height) 
            if config.pad_input:
                border_size = int((frame.shape[1] - frame.shape[0])/2)
                # padded_frame = np.zeros(shape=[frame.shape[1], frame.shape[1], 3], dtype=np.uint8)
                # padded_frame[:frame.shape[0],:frame.shape[1],] = frame
                padded_frame = cv2.copyMakeBorder(frame, border_size, border_size, 0, 0, cv2.BORDER_CONSTANT)
                frame = padded_frame
            elif config.square_input:
                left = int(frame.shape[1]/2 - frame.shape[0]/2)
                frame = frame[:, left:left+frame.shape[0]]
        ...
        if not config.no_show: 
            ...
            if config.resize_height is None:
