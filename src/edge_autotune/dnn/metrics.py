...
def compute_area_match(boxes, gt_boxes, iou_levels=[0.3, 0.5]):
    ...
    for iou_threshold in iou_levels:
        ...
        results.append({
            ...
            'avg_iou_matches': 0 if not len(avg_iou_matches) else sum(avg_iou_matches)/len(avg_iou_matches),
            'avg_iou_misses': 0 if not len(avg_iou_misses) else sum(avg_iou_misses)/len(avg_iou_misses)
        })
