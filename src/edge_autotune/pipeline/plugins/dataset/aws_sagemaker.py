...
class AWSDataset(COVADataset):
    ...
    def __init__(self, aws_config: dict, s3_config: dict, dataset_config: dict):
        ...
        self.dataset_destination = os.path.join(
            's3://', self.s3_config['bucket'], 'dataset',
    ...
    def generate_manifest(self) -> None:
        ...
        s3_objects = self.s3_config['client'].list_objects_v2(
            Bucket=self.s3_config['bucket'], Prefix=self.s3_config['annotations_prefix'])['Contents']
        for obj in s3_objects:
            _, filename = os.path.split(obj['Key'])
            with io.BytesIO() as annotations_file:
                self.s3_config['client'].download_fileobj(self.s3_config['bucket'], obj['Key'], annotations_file)
                ...
                img_dict[f'{dataset_name}-metadata'] = {
                    "class-map": {
                        str(class_id):label
                        for class_id,label in enumerate(self.dataset_config['valid_classes'])
                    },
                    "human-annotated": "no",
        ...
        with io.BytesIO() as manifest_file:
            ...
            self.s3_config['manifest'] = os.path.join(self.s3_config['annotations_prefix'], 'manifest.json')
    ...
    def generate_tfrecord(self) -> None:
        ...
        label_map = {
            str(i): c
            for i, c in enumerate(self.dataset_config['valid_classes'])
        }
