...
def train_loop(
  ...
  # is the chief.
  summary_writer_filepath = model_lib_v2.get_filepath(strategy,
      ...
      with tf.compat.v2.summary.record_if(
        ...
        if include_variables is not None:
          trainable_variables = [                                                             
              ...
              if any([var.name.startswith(prefix) for prefix in include_variables])]
        elif exclude_variables is not None: 
          trainable_variables = [
              var 
              for var in detection_model.trainable_variables
              if not any([var.name.startswith(prefix) for prefix in exclude_variables])]
        else:
          trainable_variables = detection_model.trainable_variables
...
def train(args):
  train_loop_wrapper(pipeline_config=args.pipeline_config_path, model_dir=args.model_dir, train_datasets=[args.train_dataset],
                    num_train_steps=args.num_train_steps, base_model=args.checkpoint_dir, label_map=args.label_map)
...
def main():
  ...
  parser.add_argument('--pipeline_config_path', type=str, default=os.environ.get('SM_HP_PIPELINE_CONFIG_PATH'))

  parser.add_argument('--input', type=str, default=os.environ.get('SM_INPUT_DIR'))
  parser.add_argument('--train', type=str, default=os.environ.get('SM_CHANNEL_TRAIN'))
  parser.add_argument('--eval', type=str, default=os.environ.get('SM_CHANNEL_EVAL'))
  
  parser.add_argument('--num_train_steps', type=int, default=os.environ.get('SM_HP_NUM_TRAIN_STEPS'))
  parser.add_argument('--sample_1_of_n_eval_examples', type=int, default=os.environ.get('SM_HP_SAMPLE_1_OF_N_EVAL_EXAMPLES'))
